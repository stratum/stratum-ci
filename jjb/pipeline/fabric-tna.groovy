                        ...
                        node("${BUILD_NODE}") {
                            ...
                            stage("Start Stratum on ${SWITCH_NAME}") {
                                ...
                                sh returnStdout: false, label: "Starting Stratum with Image ${DOCKER_IMAGE}:${DOCKER_IMAGE_TAG}", script: """
                                    ...
                                    sshpass -p $SWITCH_CREDS_PSW ssh $SWITCH_CREDS_USR@$SWITCH_IP "tmux send-keys -t CI.0 ENTER 'DOCKER_IMAGE=${REGISTRY_URL}/${DOCKER_IMAGE} DOCKER_IMAGE_TAG=${DOCKER_IMAGE_TAG} CHASSIS_CONFIG=${CONFIG_DIR}/${SWITCH_NAME}/chassis_config.pb.txt ./restart-stratum.sh --bf-switchd-background=false' ENTER"
                                    sleep 60
                                """
                            }
